...
class TestBasicSmtp(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        subprocess.Popen(['./tests/smtp_server/test_smtp/bin/test_smtp_protocol', str(PORT)]# , stdout=subprocess.DEVNULL
        )
        sleep(.1)

    @classmethod
    def tearDownClass(cls):
        subprocess.Popen(['/bin/bash', '-c', 'killall test_smtp_protocol'], stdout=subprocess.DEVNULL)
